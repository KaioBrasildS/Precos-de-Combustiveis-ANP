...
# Função para executar o modelo com os parâmetros `X`, `y` e `use_scaler`
def executar_modelo(X, y, use_scaler=True, kernel_type='rbf', **kernel_params):
    # Certifique-se de que os dados sejam numéricos
    X = X.apply(pd.to_numeric, errors='coerce').dropna()
    y = pd.to_numeric(y, errors='coerce').dropna()

    # Aplicar o StandardScaler opcionalmente
    if use_scaler:
    ...
    # Treinar o modelo de forma online com os dados
    for x_new, y_new in zip(X_dicts, y):
        ridge_sgd_torch.partial_fit(x_new, y_new)

    # Prever com os dados fornecidos
    y_pred = []
    for x in X_dicts:
        prediction = ridge_sgd_torch.predict(x)
        y_pred.append(prediction)

    # Previsões indexadas
    y_indexed = pd.Series(y_pred, index=y.index)

    # Exibir resultados
    print("Previsões para os dados:", y_pred)
    plt.figure(figsize=(16, 6))
    plt.plot(y, label='Valor Real')
    plt.plot(y_indexed, label='Valor Previsto', linestyle='--')
...
# executar_modelo(X, y, use_scaler=True, kernel_type='rbf')
